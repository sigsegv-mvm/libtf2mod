...
PATCH_CHECK
{
	...
	uintptr_t off4_call1 = CALC_RELJMP(check4_func, 0x2454,
		CAttributeManager_AttribHookValue_int);
	...
	size_t check4_base = 0x242c;
	uint8_t check4[] = {
		0xc7, 0x44, 0x24, 0x10, 0x01, 0x00, 0x00, 0x00, // +242C  mov DWORD PTR [esp+0x10],0x1
		0xc7, 0x44, 0x24, 0x0c, 0x00, 0x00, 0x00, 0x00, // +2434  mov DWORD PTR [esp+0xc],0x0
		0x89, 0x4c, 0x24, 0x08,                         // +243C  mov DWORD PTR [esp+0x8],ecx
		0xc7, 0x44, 0x24, 0x04, CONV_LE(off_str2)       // +2440  mov DWORD PTR [esp+0x4],str "halloween_green_flames"
		0xc7, 0x04, 0x24, 0x00, 0x00, 0x00, 0x00,       // +2448  mov DWORD PTR [esp],0x0
		0xe8, CONV_LE(off4_call1)                       // +244F  call CAttributeManager::AttribHookValue<int>
	};
...
PATCH_APPLY
{
	...
	func_write(CTFPlayerShared_ConditionGameRulesThink, 0x2444, 4, str.bytes);
