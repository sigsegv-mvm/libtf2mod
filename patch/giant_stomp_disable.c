...
PATCH_CHECK
{
	...
	uintptr_t off2 = vtable_find_offset("_ZTV11CBaseEntity",
		CBaseEntity_IsBaseObject);
	
	uintptr_t off3 = vtable_find_offset("_ZTV11CBaseObject",
		CBaseObject_GetType);
	...
	uint8_t check1[] = {
		...
		0xff, 0x90, CONV_LE(off2)           // +00B9  call DWORD PTR [eax+IsBaseObject] CBaseEntity::IsBaseObject
		0x84, 0xc0,                         // +00BF  test al,al
		0x0f, 0x84, 0x79, 0xff, 0xff, 0xff, // +00C1  je -0x87
		0x8b, 0x06,                         // +00C7  mov eax,DWORD PTR [esi]
		0x89, 0x34, 0x24,                   // +00C9  mov DWORD PTR [esp],esi
		0xff, 0x90, CONV_LE(off3)           // +00CC  call DWORD PTR [eax+GetType] CBaseObject::GetType
